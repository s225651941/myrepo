...
###data.frame Creates a table-like structure in R.
```
```{r}
...
###boxplot is a base R plotting function that shows
```
...
###as.numeric gets the numeric frequency values
###rbind makes the two data frames row-wise
```
```{r}
...
###barplot creates a simple bar chart in base R.
```
...
###uco Calculates codon usage for a DNA sequence
```
```{r}
...
###rscu defines a function to calculate RSCU from a codon frequency vector.
```
```{r}
...
```
```{r}
# Translate CDS to amino acid sequences
ecoli_prot <- lapply(ecoli_cds, translate)
bifido_prot <- lapply(bifido_cds, translate)

# Concatenate all proteins into single string
...
for (k in ks) {
  ...
  df <- data.frame(
    ...
    Ecoli = as.numeric(ecoli_freq[names(bif_freq)]), # match k-mers
    stringsAsFactors = FALSE
  )
  
  # Calculate log2 ratio
  df$log2_ratio <- log2((df$Bifido + 1e-6) / (df$Ecoli + 1e-6)) # add small pseudo-count to avoid division by zero
...
K-mer analysis indicates that there are specific short amino acid motifs (3-5 bases) that are grossly over- or under-represented in Bifidobacterium relative to E. coli.

The over-represented motifs like GPG or PAA can be linked to structural plasticity in proteins because the Bifidobacterium enzymes tend to be active in the anaerobic gut where folding and stability are affected by the pH and redox conditions (Turroni et al., 2010).
